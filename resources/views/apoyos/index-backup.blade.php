...
<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <h1 class="text-2xl font-bold text-slate-900 mb-4">Debug Info</h1>
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div class="bg-blue-50 p-3 rounded border border-blue-200">
                    <p class="font-semibold text-blue-900">Usuario</p>
                    <p class="text-blue-700">{{ $user->name ?? 'No autenticado' }}</p>
                </div>
                <div class="bg-green-50 p-3 rounded border border-green-200">
                    <p class="font-semibold text-green-900">Total Apoyos</p>
                    <p class="text-green-700">{{ count($apoyos ?? []) }} cargados</p>
                </div>
                <div class="bg-yellow-50 p-3 rounded border border-yellow-200">
                    <p class="font-semibold text-yellow-900">Es Beneficiario</p>
                    <p class="text-yellow-700">{{ $isBeneficiario ? 'Sí' : 'No' }}</p>
                </div>
                <div class="bg-purple-50 p-3 rounded border border-purple-200">
                    <p class="font-semibold text-purple-900">Puede Editar</p>
                    <p class="text-purple-700">{{ $canEdit ? 'Sí' : 'No' }}</p>
                </div>
            </div>
        </div>
        ...
        @if(empty($apoyos) || count($apoyos) === 0)
            <div class="bg-yellow-50 border-2 border-yellow-300 rounded-lg p-6 text-center">
                <p class="text-xl font-bold text-yellow-800 mb-2">⚠ No hay apoyos</p>
                <p class="text-yellow-700">La variable $apoyos está vacía o no fue pasada a la vista</p>
            </div>
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($apoyos as $apoyo)
                    <div class="bg-white border border-slate-200 rounded-lg p-4 hover:shadow-lg transition">
                        <h3 class="font-bold text-slate-900 mb-2">{{ $apoyo->nombre_apoyo }}</h3>
                        <p class="text-sm text-slate-600 mb-2">{{ $apoyo->tipo_apoyo }}</p>
                        <p class="text-xs text-slate-500">Vigencia: {{ $apoyo->fechaInicio ?? '-' }} al {{ $apoyo->fechafin ?? '-' }}</p>
                    </div>
                @endforeach
            </div>
        @endif
